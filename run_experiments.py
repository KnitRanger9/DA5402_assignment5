...
def create_combined_datasets():
    ...
    try:
        ...
        subprocess.run(['dvc', 'checkout', 'data/partition_v1.dvc'], check=True)
        subprocess.run(['dvc', 'checkout', 'data/partition_v2.dvc'], check=True)
        subprocess.run(['dvc', 'checkout', 'data/partition_v3.dvc'], check=True)
        
        # Create v1+v2 combined dataset
        v1v2_dir = 'data/partition_v1v2'
        if os.path.exists(v1v2_dir):
            subprocess.run(['rm', '-rf', v1v2_dir], check=True)
        
        subprocess.run(['mkdir', '-p', v1v2_dir], check=True)
        
        # Copy class directories and images
        class_names = os.listdir('data/partition_v1')
        for class_name in class_names:
            class_dir = os.path.join(v1v2_dir, class_name)
            subprocess.run(['mkdir', '-p', class_dir], check=True)
            
            # Copy images from v1
            subprocess.run([
                'cp', 
                f'data/partition_v1/{class_name}/*',
                class_dir
            ...
            subprocess.run([
                'cp', 
                f'data/partition_v2/{class_name}/*',
                class_dir
            ], check=True, shell=True)
        
        # Create v1+v2+v3 combined dataset
        v1v2v3_dir = 'data/partition_v1v2v3'
        if os.path.exists(v1v2v3_dir):
            subprocess.run(['rm', '-rf', v1v2v3_dir], check=True)
        
        subprocess.run(['mkdir', '-p', v1v2v3_dir], check=True)
        
        # Copy class directories and images
        for class_name in class_names:
            class_dir = os.path.join(v1v2v3_dir, class_name)
            subprocess.run(['mkdir', '-p', class_dir], check=True)
            ...
            subprocess.run([
                'cp', 
                f'data/partition_v3/{class_name}/*',
                class_dir
            ], check=True, shell=True)
        
        # Add combined datasets to DVC
        subprocess.run(['dvc', 'add', v1v2_dir], check=True)
        subprocess.run(['dvc', 'add', v1v2v3_dir], check=True)
        
        # Add to Git
        subprocess.run(['git', 'add', f'{v1v2_dir}.dvc'], check=True)
        subprocess.run(['git', 'add', f'{v1v2v3_dir}.dvc'], check=True)
        
        # Commit changes
        subprocess.run(['git', 'commit', '-m', 'Add combined dataset partitions'], check=True)
